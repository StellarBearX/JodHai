...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    fetchChatHistory(user.lineUserId)
      .then((logs) => {
  ...
  const handleSend = async (text: string = input.trim()) => {
    ...
    setInput(''); stopWriting(); setSending(true);
    ...
    try {
      if (img) {
        ...
        if (isChatQuestion(res)) push(mk({ kind: 'text', role: 'bot', text: res.question }));
        else if (isChatAnswer(res)) push(mk({ kind: 'answer', role: 'bot', answer: res.answer, emotion: res.emotion }));
        else if (isChatEdited(res)) { push(mk({ kind: 'edited', role: 'bot', tx: { ...res.transaction, createdAt: new Date(res.transaction.createdAt) }, message: res.message, emotion: res.emotion })); triggerSuccess(); loadDashboard(); loadTransactions(); }
        else if (isChatDeleted(res)) { push(mk({ kind: 'deleted', role: 'bot', message: res.message, emotion: res.emotion })); loadDashboard(); loadTransactions(); }
        else { push(mk({ kind: 'tx', role: 'bot', tx: { ...res.transaction, createdAt: new Date(res.transaction.createdAt) }, usedTraining: res.usedTraining, autoLearned: res.autoLearned, message: res.message, emotion: res.emotion })); triggerSuccess(); loadDashboard(); loadTransactions(); }
      } else {
  ...
  if (!loaded) return (
    <div className="flex flex-col h-full items-center justify-center gap-3" style={{ background: 'var(--bg)' }}>
      <NongJodHai state="idle" size={72} />
      <p className="text-sm font-medium" style={{ color: 'var(--text-3)' }}>กำลังโหลดประวัติสนทนา...</p>
    </div>
  );
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: 'var(--bg)' }}>
      ...
      {/* ── Chat header ── */}
      <div
        className="px-4 pt-3 pb-3 flex items-center gap-3 flex-shrink-0"
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-sm font-bold" style={{ color: 'var(--text-1)' }}>น้องจดให้</h1>
          <p className="text-xs font-medium" style={{ color: 'var(--income)' }}>● ออนไลน์ · Gemini 2.5 Flash</p>
        </div>
        {user && (
          <span
        ...
        {img && (
          ...
          >
            <div className="relative">
              <img
                src={img.preview} alt="ตัวอย่างรูปที่จะส่ง"
                className="w-14 h-14 object-cover rounded-xl"
                style={{ border: '1.5px solid var(--border)', boxShadow: 'var(--shadow-xs)' }}
              />
              <button
                onClick={() => setImg(null)}
                className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full flex items-center justify-center focus-visible:outline-none"
            ...
            <p className="text-xs" style={{ color: 'var(--text-3)' }}>กดส่งเพื่อให้น้องจดให้อ่านสลิปค่า</p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {/* ── Suggestions ── */}
      {msgs.length <= 2 && (
        <div className="px-4 py-2 flex-shrink-0" style={{ background: 'var(--surface)', borderTop: '1px solid var(--border)' }}>
          <p className="text-xs mb-2 font-medium" style={{ color: 'var(--text-3)' }}>ลองพิมพ์แบบนี้ค่า</p>
          <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
            {SUGGESTIONS.map((s) => (
              <button
                ...
                disabled={sending}
                className="flex-shrink-0 text-xs px-3 py-1.5 rounded-full font-semibold transition-all active:scale-95 disabled:opacity-50 focus-visible:outline-none"
      ...
      <div
        ...
        style={{ background: 'var(--surface)', borderTop: '1px solid var(--border)' }}
      >
        <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={handleImagePick} aria-hidden="true" />
        ...
        <motion.button
          ...
          className="icon-wrap flex-shrink-0 transition-colors disabled:opacity-40 focus-visible:outline-none"
        ...
        <motion.button
          ...
          style={{
            ...
            boxShadow: '0 2px 10px rgba(163,78,48,0.30)',
          }}
        >
          {sending
            ? <Loader2 size={17} className="animate-spin" color="white" />
  ...
  if (m.kind === 'image') {
    return (
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex gap-2 items-end flex-row-reverse">
        {m.preview
          ? <img src={m.preview} alt="สลิปที่ส่ง" className="max-w-[55%] rounded-2xl" style={{ borderBottomRightRadius: 4, boxShadow: 'var(--shadow-md)' }} />
          : <div className="px-4 py-2.5 rounded-2xl text-sm italic" style={{ background: 'linear-gradient(135deg, var(--brand-btn), var(--brand-dark))', color: 'white', borderBottomRightRadius: 4 }}>📸 ส่งรูปใบเสร็จ</div>
        }
      </motion.div>
    );
  }
...
function TxCard({ m, onNavigate }: { m: TxMsg; onNavigate: () => void }) {
  ...
  return (
    ...
    >
      <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 mb-0.5 text-sm" style={{ background: 'var(--brand-dim)' }} aria-hidden="true">
        🤖
      </div>
      <div
        className="max-w-[88%] overflow-hidden"
        style={{ borderRadius: '18px 18px 18px 4px', border: '1.5px solid var(--border)', boxShadow: 'var(--shadow-md)', background: 'var(--surface)' }}
      >
        <div className="px-4 pt-3 pb-2">
          <p className="text-sm leading-relaxed" style={{ color: 'var(--text-1)' }}>{message}</p>
        </div>

        <div className="mx-3 mb-3 rounded-xl overflow-hidden" style={{ background: 'var(--surface-2)', border: '1px solid var(--border)' }}>
          <div
            className="px-3 py-2 flex items-center justify-between"
            style={{ background: isExp ? 'var(--expense-bg)' : 'var(--income-bg)' }}
          >
            <span className="text-xs font-bold px-2 py-0.5 rounded-md" style={{ background: 'var(--surface)', color: isExp ? 'var(--expense)' : 'var(--income)' }}>
              {isExp ? 'รายจ่าย' : 'รายรับ'}
            </span>
            <span className="text-sm font-extrabold" style={{ color: isExp ? 'var(--expense)' : 'var(--income)' }}>
          ...
          <div className="px-3 py-2.5 flex items-center gap-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold truncate" style={{ color: 'var(--text-1)' }}>{tx.note ?? tx.category}</p>
          ...
          {(usedTraining || autoLearned) && (
            <div className="px-3 pb-2.5 flex gap-1.5 flex-wrap">
              {usedTraining && <span className="text-xs px-2 py-0.5 rounded-md font-semibold" style={{ background: '#EDE9FE', color: '#5B21B6' }}>⚡ cache</span>}
              {autoLearned && <span className="text-xs px-2 py-0.5 rounded-md font-semibold" style={{ background: 'var(--income-bg)', color: 'var(--income)' }}>🧠 เรียนรู้แล้ว</span>}
            </div>
          )}
        </div>
        ...
        <div className="px-4 pb-3 flex justify-end">
          <button
            onClick={onNavigate}
            className="text-xs font-semibold focus-visible:outline-none"
    ...
    >
      ...
      >
        <div className="px-4 pt-3 pb-2 flex items-start gap-2 flex-wrap">
          <span className="text-xs px-2 py-0.5 rounded-md font-semibold flex-shrink-0" style={{ background: 'var(--warning-bg)', color: 'var(--warning)' }}>
            ✏️ แก้ไขแล้ว
          </span>
          <p className="text-sm leading-relaxed w-full" style={{ color: 'var(--text-1)' }}>{message}</p>
        </div>

        <div className="mx-3 mb-3 rounded-xl overflow-hidden" style={{ background: 'var(--surface-2)', border: '1px solid var(--border)' }}>
          <div className="px-3 py-2 flex items-center justify-between" style={{ background: isExp ? 'var(--expense-bg)' : 'var(--income-bg)' }}>
            <span className="text-xs font-bold px-2 py-0.5 rounded-md" style={{ background: 'var(--surface)', color: isExp ? 'var(--expense)' : 'var(--income)' }}>
              {isExp ? 'รายจ่าย' : 'รายรับ'}
            </span>
            <span className="text-sm font-extrabold" style={{ color: isExp ? 'var(--expense)' : 'var(--income)' }}>
        ...
        <div className="px-4 pb-3 flex justify-end">
          <button onClick={onNavigate} className="text-xs font-semibold focus-visible:outline-none" style={{ color: 'var(--brand-dark)' }}>
            ดูใน ประวัติ →
          </button>
        </div>
      </div>
